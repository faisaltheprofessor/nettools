    ...
    <flux:card class="h-full max-h-screen flex flex-col pb-4">
            ...
            <div class="flex items-center justify-center">
                ...
                <flux:tooltip toggleable>
                    ...
                    <flux:tooltip.content class="max-w-[20rem] space-y-2">
                        <p>Folgende Filter werden für die Ausgabe der Karteileichen herangezogen: keine Benutzersperre, letzter Login älter als 180 Tage (ab aktuelles Datum), Ausschluss "DeaktivierteUser", User-ID Range p10000-p19999.</p>
                        <p>Vor der endgültigen Deaktivierung dieser Benutzer ist dringend Rücksprache mit den jeweiligen Fachbereichen zu halten. Die Ausgabe ist darüber hinaus bitte immer mit der "User-ID" nachzuprüfen!</p>
        ...
        @if (!empty($exportOutput) && $exportMode === 'table')
            ...
            >
                ...
                <div class="relative shadow-md sm:rounded-lg border border-gray-300 dark:border-gray-700 flex-1 min-h-0 flex flex-col">
                    <div class="flex-1 min-h-0 overflow-y-auto overflow-x-auto">
                        <table x-ref="expTable" class="w-full text-sm text-left text-gray-600 dark:text-gray-200 bg-gray-50 dark:bg-gray-900/20">
                            <thead class="sticky top-0 z-10 text-xs text-gray-700 dark:text-gray-100 uppercase bg-gray-100 dark:bg-gray-800">
                            <tr>
                                <th class="!pl-10 pr-2 py-3">
                                    <div class="inline-flex items-center gap-1">
                                        <button type="button" class="inline-flex items-center gap-1 cursor-pointer" x-on:click="toggle('uid')">
                                            P-ID
                                            <template x-if="sortKey==='uid' && asc"><flux:icon.arrow-up-wide-narrow class="size-3.5"/></template>
                                            <template x-if="sortKey==='uid' && !asc"><flux:icon.arrow-down-wide-narrow class="size-3.5"/></template>
                                        </button>
                                ...
                                <template x-for="(c, i) in cols" :key="'h-'+c">
                                    <th class="px-2 py-3">
                                        <div class="inline-flex items-center gap-1">
                                            <button type="button" class="inline-flex items-center gap-1 cursor-pointer" x-on:click="toggle(c)">
                                                <span x-text="names[c] ?? c"></span>
                                                <template x-if="sortKey===c && asc"><flux:icon.arrow-up-wide-narrow class="size-3.5"/></template>
                                                <template x-if="sortKey===c && !asc"><flux:icon.arrow-down-wide-narrow class="size-3.5"/></template>
                                            </button>
                                            <span role="button" tabindex="0" title="Spalte kopieren"
                ...
                <div class="mt-3 pt-2 border-t border-gray-300 dark:border-gray-700">
...
function exportTable({rows, cols, names}){
    return {
        ...
        toggle(k){ this.sortKey===k ? this.asc = !this.asc : (this.sortKey=k, this.asc=true) },
        val(v,k){
            if(k==='uid' && typeof v==='string'){ const m=v.match(/^p(\d+)$/i); return m?parseInt(m[1],10):v }
            if(k==='logintime' && typeof v==='string'){ const t=v.split('.'); if(t.length===3){ return new Date(t[2], t[1]-1, t[0]).getTime() } }
