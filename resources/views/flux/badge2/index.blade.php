...
@php
    ...
    $classes = Flux::classes()
        ->add('inline-flex items-center font-medium whitespace-nowrap w-fit')
        ->add($insetClasses)
        ->add('[print-color-adjust:exact]')
        ->add(match ($size) {
            'lg' => 'text-sm py-1.5 **:data-flux-badge-icon:me-2',
            default => 'text-sm py-1 **:data-flux-badge-icon:me-1.5 w-fit',
...
{{--    COPY BUTTON--}}
   {{-- <span
        ...
        x-data="{ copied: false }"
        x-on:click="
        const root  = $el.closest('[data-flux-badge]');
        const clone = root.cloneNode(true);

        // Remove everything that's NOT the slot (icons + this button)
            ...
            try {
                if (navigator.clipboard && window.ClipboardItem) {
                    await navigator.clipboard.write([
                        new ClipboardItem({
                            'text/html': new Blob([html], { type: 'text/html' }),
                            'text/plain': new Blob([clone.textContent.trim()], { type: 'text/plain' }),
                        })
                    ]);
                } else {
